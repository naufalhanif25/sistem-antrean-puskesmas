...
export default function DaftarAntreanPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Map dokter sesuai cluster
      const clusterData = data.doctors.map((d: any) => ({
        cluster: d.cluster,
        doctorName: d.nama,
      }));
      ...
      console.error(error);
      showToast("Gagal ambil data antrean", "error");
    }
  };
  ...
  const activeClusters = Array.from(
    new Set(
      queueData.filter((p) => p.status !== "Selesai").map((p) => p.cluster),
    ),
  );
  ...
  const handleExportCSV = useCallback(() => {
    ...
    const headers = ["No", "Nomor Antrean", "Nama Pasien", "Status", "Cluster"];
    ...
    link.href = url;
    link.setAttribute("download", `antrean_${new Date().toISOString()}.csv`);
  ...
  const handleLewati = useCallback(async (id: number) => {
    try {
      ...
      console.error(error);
      showToast("Gagal lewati antrean", "error");
    }
  }, []);
      ...
      >
        ...
        <div className="px-6 pb-6">
          <div className="mb-6">
            ...
            >
              Kelola antrean pasien yang sedang berjalan, termasuk memanggil
              atau melewati antrean.
            </p>
          </div>
          ...
          >
            ...
            <div
              ...
              style={{
                borderTop: isDark ? "1px solid #2A2A2A" : "1px solid #E5E7EB",
              }}
            >
              <p
                className="text-sm transition-colors duration-300"
              ...
              >
                ...
                {Math.min(endIndex, queueData.length)} of {queueData.length}{" "}
                Pasien
              </p>
